...
const WayseraJourney = (() => {
    ...
    class Session {
        ...
        close() {
            ...
            // A socket still CONNECTING would otherwise be dropped here and
            // then open, unobserved, after the user has left.
            if (socket.readyState !== WebSocket.OPEN) {
                try { socket.close(); } catch (error) { /* already closing */ }
                return;
            }

            // Sealing is async. The old code nulled this.socket and closed the
            // connection on the same tick, so rawSend always found a dead
            // socket and the bye frame never left — peers waited out the full
            // 120s roster timeout instead. Seal against the captured socket,
            // then close.
            if (!this.key) {
                socket.close();
                return;
            ...
            WayseraCrypto.seal(this.key, { type: 'bye', memberId: this.memberId, ts: Date.now() })
                .then((frame) => {
        ...
        async applyMessage(message) {
            switch (message.type) {
                case 'hello': {
                    const isNew = this.touchMember(message.memberId, { name: message.name });
                    if (!message.reply && isNew) {
        ...
        /** Record a sighting. Returns true when this member was not already known. */
        touchMember(memberId, patch) {
            const known = this.members.has(memberId);
        ...
        async handleHandshake(raw) {
            ...
            if (message.type === 'key_request') {
                ...
                // Refusal is remembered. A joiner without a key re-asks on every
                // heartbeat, so without this the prompt returns every three
                // seconds for the rest of the journey and "Not now" is a snooze.
                // Still silent: nothing goes back to the requester.
                if (this.deniedRequests.has(message.memberId)) return;

                // First request for a member wins. The map is keyed by memberId
                // and approveKeyRequest re-reads it, so a later request under
                // the same id used to replace the one the human actually saw —
                // letting a substituted public key be the one the journey key
                // gets wrapped for. A repeat carrying a different key is
                // therefore dropped, not merged.
                const already = this.pendingRequests.get(message.memberId);
                if (already) {
                    if (already.pub !== message.pub) return;
                    return; // Same request re-sent by the heartbeat; card is up.
                }
        ...
        denyKeyRequest(memberId) {
            ...
            // them the code was right. Remembered, though, or the heartbeat
            // simply asks again three seconds later.
            this.pendingRequests.delete(memberId);
            this.deniedRequests.add(memberId);
        }
