...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    osascript -e "tell application \"Terminal\"
        do script \"cd '$PROJECT_DIR/backend' && source .venv/bin/activate && echo 'Starting relay...' && uvicorn main:app --reload --host 0.0.0.0 --port 8000\"
        activate
    end tell"
    ...
    osascript -e "tell application \"Terminal\"
        do script \"cd '$PROJECT_DIR/frontend' && echo 'Starting web client...' && python3 -m http.server 3000\"
        activate
    end tell"
    ...
    if command -v gnome-terminal &> /dev/null; then
        ...
        gnome-terminal --tab --title="Waysera web" -- bash -c "cd '$PROJECT_DIR/frontend' && echo 'Starting web client...' && python3 -m http.server 3000; exec bash"
        ...
        xterm -hold -e "cd '$PROJECT_DIR/frontend' && echo 'Starting web client...' && python3 -m http.server 3000" &
        ...
        konsole --new-tab -e bash -c "cd '$PROJECT_DIR/frontend' && echo 'Starting web client...' && python3 -m http.server 3000; exec bash" &
    else
