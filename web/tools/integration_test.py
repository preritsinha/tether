...
async def run() -> list[str]:
    ...
    created = await host.json_eval(f"""
        (async () => {{
            ...
            const code = await createJourney();
    ...
    # Both pages share one Chrome profile, so they share localStorage and the
    # host's remembered name. Land on the plain page first and clear it, or the
    # invite link auto-joins this page under the host's identity and the roster
    # ends up with two of them. Separate devices never collide this way.
    await guest.navigate(f"{ORIGIN}/index.html")
    await guest.evaluate("localStorage.removeItem('waysera.name')")
